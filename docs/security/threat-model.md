...
| Dashboard-link leakage | Temporary local admin access | 90-second one-use fragment, immediate history scrub, HttpOnly signed session, same-origin/CSRF checks, loopback-only ingress |
...
| Paid-model route drift | Unexpected credit spend | Runtime accepts only exact `:free` live-catalog IDs with zero prompt/completion/request prices, caps the provider fallback array, checks the actual selected model and zero returned cost, and keeps an atomic PostgreSQL daily cap |
| Privacy-incompatible quality routing | Résumé retention/training despite a stronger benchmark | Default routing intersects free candidates with the live active-ZDR endpoint inventory and retains `data_collection=deny`; benchmark rank never weakens privacy automatically |
...
| Unsolicited creator outreach | Privacy/legal/reputation damage | Official metadata may yield unreviewed business-contact candidates; operator verifies provenance/basis and authorizes separately; each send exact-approved; opt-out/bounce suppresses durably |
| Misattributed or hostile creator descriptions | Wrong recipient, misleading fit, injection | Bounded source excerpts, business-context candidate heuristic, escaped display, formula-safe CSV, explicit research gaps; descriptions never become instructions or authorization |
...
The script bearer token is still service-wide and browser sessions are
single-operator rather than per-user identity; image digests ensure immutability
but not upstream trust; audit storage shares database role with application; public
