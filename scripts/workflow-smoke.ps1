...
try {
    ...
    $probeSource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'workflow-smoke.py') -Raw
...
exec(compile(source, 'scripts/workflow-smoke.py', 'exec'), {'__name__': '__main__', 'MIGRATIONS': migrations})
"@
    $bootstrap | docker compose exec -T control-api python -
    if ($LASTEXITCODE -ne 0) { throw 'Disposable PostgreSQL workflow smoke failed' }
}
finally {
    Pop-Location
}
