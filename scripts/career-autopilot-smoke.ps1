...
try {
    ...
    $probeSource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'career-autopilot-smoke.py') -Raw
...
exec(compile(source, 'scripts/career-autopilot-smoke.py', 'exec'), {'__name__': '__main__', 'MIGRATIONS': migrations})
"@
    $bootstrap | docker compose exec -T control-api python -
    if ($LASTEXITCODE -ne 0) { throw 'Disposable PostgreSQL career autonomy/tracking smoke failed' }
}
finally {
    Pop-Location
}
