...
try {
    ...
    $probeSource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'scheduler-smoke.py') -Raw
...
exec(compile(source, 'scripts/scheduler-smoke.py', 'exec'), {'__name__': '__main__', 'MIGRATIONS': migrations})
"@
    $bootstrap | docker compose exec -T control-api python -
    if ($LASTEXITCODE -ne 0) { throw 'Disposable PostgreSQL scheduler smoke failed' }
}
finally {
    Pop-Location
}
